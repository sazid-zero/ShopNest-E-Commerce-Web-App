...
async function getUser(req) {
    ...
    try {
        const decoded = await __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$firebase$2d$admin$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["auth"].verifyIdToken(token);
        return decoded;
    } catch (e) {
        return null;
    }
...
async function GET(req) {
    ...
    try {
        const result = await client.query(`SELECT c.product_id, c.quantity, p.name, p.price, p.image_url 
       FROM cart c
...
async function POST(req) {
    ...
    try {
        if (quantity <= 0) {
            await client.query('DELETE FROM cart WHERE user_id = $1 AND product_id = $2', [
                user.uid,
                productId
            ]);
        } else {
            await client.query(`INSERT INTO cart (user_id, product_id, quantity)
         VALUES ($1, $2, $3)
...
async function PUT(req) {
    ...
    try {
        ...
        for (const item of items){
            await client.query(`INSERT INTO cart (user_id, product_id, quantity)
         VALUES ($1, $2, $3)
         ON CONFLICT (user_id, product_id) DO UPDATE SET quantity = GREATEST(cart.quantity, $3)`, // Strategy: take logic to keep max, or just sum? 
...
//# sourceMappingURL=%5Broot-of-the-server%5D__aa99e841._.js.map